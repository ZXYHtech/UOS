    ...
    *,
    ...
    if not has_remote:
        ...
        data = _build_handoff(
            ...
            next_action=next_action, context_refs=context_refs,
        )
        ...
        for rel in artifacts:
            ...
            if target.exists() or target.is_symlink():
                if source.is_file() and target.is_file() and source.read_bytes() == target.read_bytes():
                    ...
                    raise HandoffError(f"TARGET_PATH_CONFLICT: canonical partial artifact differs: {rel}")
            else:
...
def read_handoff(
    ...
    *,
    ...
    result["warning"] = "UNVERIFIED_PARTIAL_WORK: re-run task Acceptance before final completion."
    return result
