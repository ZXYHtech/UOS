...
- expected reviewed head at handoff: `7a8e0adcc0bf4f8fddba4688802ef2486f18f56c`
- PR: `ZXYHtech/inventory#3`
- PR direct state at handoff: `mergeable=true`, `rebaseable=true`, `mergeable_state=clean`, Draft
...
```text
7a8e0adcc0bf4f8fddba4688802ef2486f18f56c
```
...
15. deployment safety regression proves `setup_server.sh` protects the entire runtime `data/` tree from `rsync --delete`;
16. deployment safety regression proves setup health-gates release publication and backup timer enablement;
17. deployment safety regression proves update quiesces writers before code/schema cutover and restores timer only after health;
18. pre-upgrade recovery bundle is bound to the old deployed release and captures available deployment config;
19. existing auth/security, core workflow, recognition and warehouse-efficiency regressions pass;
20. shell/client checks run according to tool availability/strict flags.
